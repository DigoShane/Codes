...
mol = gto.Mole() # create a molecule object
mol.build(
    verbose = 0,
    atom = '''8  0  0.     0
              1  0  -0.757 0.587
...
mf = scf.RHF(mol) # specify the solver to be Restricted Hartree Fock.
mf.conv_tol = 1e-1 # convergence threshold
mf.kernel() # runs the calculations.
mo_init = mf.mo_coeff # ??xDx?? This sounds like a way to extract the solution.
mocc_init = mf.mo_occ # Get orbital occupancy.

mf = scf.RHF(mol).newton() # Running second order SCF.
energy = mf.kernel(mo_init, mocc_init) # running the calculations.
