...
#----------------------------------------------------------------------------------
#What i am trying to test is the ability to extend this code to incorporate larger dimensions.
#Before writing the output, i will extend the mesh to a larger domain.
#======================================================================================================
...
Lx_new=110
print("New discretization=", int(Lx_new)*10)
mesh_new = fe.IntervalMesh(1100,0,Lx_new)
V_new = FunctionSpace(mesh_new, "Lagrange", 2) #Try to be consistent with the Element space.
A_new = Function(V_new)
u_new = Function(V_new)
plot(u)
plt.title(r"$u_{old}(x)$",fontsize=26)
plt.show()
plot(A)
plt.title(r"$A_{old}(x)e_2$",fontsize=26)
plt.show()

#Interpolate the existing solution onto the new mesh
u_new = 1-project(1-u, V_new)#, DirichletBC(V_new, 1, fe.IntervalMesh(100,Lx,Lx_new) ))
A_new = project(A, V_new)#, DirichletBC(V_new, 0, fe.IntervalMesh(100,Lx,Lx_new) ))
plot(u_new)
plt.title(r"$u_{new}(x)$",fontsize=26)
plt.show()
plot(A_new)
plt.title(r"$A_{new}(x)e_2$",fontsize=26)
plt.show()

##This is the old way to save the file. Check new way below.
...
##Save solution on extended domain in a .xdmf file
Aur_out = XDMFFile('test-0-Constraint4.xdmf')
Aur_out.write_checkpoint(A, "A", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
Aur_out.close()
Aur_out = XDMFFile('test-1-Constraint4.xdmf')
Aur_out.write_checkpoint(u, "u", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
Aur_out.close()
with open("test-2-Constraint4.txt", "w") as file:
    print(float(r), file=file)
